...
- Interactions with the Deck are not being correctly 'locked' in all relevant scenarios. This is also reflected in the tech debt section as a system which needs reworking in its entirety; the bug is for specific known problems.
  - During resolution of a Battle round, the Deck can trigger additional rounds repeatedly without waiting for the current one to be completed.
- (observed once while running WebGL build 0.0.1 in Chrome) During cleanup of a Trap encounter with 4 score cards: <King of Spades | 02 of Diamonds | 06 of Clubs | 04 of Spades> the King of Spades stayed in place while the other score cards and encounter cards were correctly returned to the Deck. This occurred while spamming the D key to check for any issues caused by failing to handle input correctly - may or may not be related. Debug messages in the DevTools console indicate that the 'virtual' card was returned to the Deck as expected but movement was not recorded (all other cards reported completion of their movement).
  ...
  - Similar observations while testing 0.0.6 changes (pre-release) in Unity Editor, and build 0.0.6 in Chrome. A couple of times in Monster battles, text was left on screen prompting the user to click the deck even after a new scoring round had already started; this resolved itself in the next scoring round each time it happened. On one occasion while the new game process was finding a character card, a rejected candidate stayed behind when the other cards were returned to the deck; this happened in the browser and blocked the process from continuing. The issue remains highly intermittent with no obvious cause.
  - Observed again in build 0.0.7, first during Build and Run, then again after deploying online for testing, both times during character card selection.
  - Card transfer between CardZones needs an overhaul to support sound effects. I'll try and address this bug while working on that upgrade.
...
- Selecting a card during a 'trapped Treasure' encounter, and then abandoning the Treasure, keeps the card selected and playable in the Town phase.
  - As part of general Encounter teardown, all cards in the player's hand (including active cards) should be deselected.
...
- Refactor card dealing logic to trigger the sound effect as each card leaves the deck. This is currently complicated by the receiving CardZone having control over movement of new cards, and Deck typically passing a list of cards rather than one at a time (which makes it easier to inform listeners when all the expected cards have finished moving). A particularly problematic case is Player.Hand, which may receive cards from either Deck or StagingArea and should not play the 'dealt card' sound effect in the latter case.
  - Note: while working on this refactor, tighten card transfer logic to try and address the 'stuck card' bug.
...
- Improve GameState locking/unlocking and use of Next()/Encounter.Advance() as that pattern is not so broadly applicable as it was initially assumed to be; q.v. traps on chests and paying healer fees.
  - Ensure the Deck can't be repeatedly activated e.g. dealing multiple lots of score cards in a single Encounter round. This was observed while migrating to the new Unity Input System, where pressing the D key caused repeated, rapid triggering of `GameController.RegisterInteractionWithDeck()` during Town phases and Battle encounters with unexpected and undesirable effects. This is a problem with specific scenarios but highlights a weakness which will similarly affect other scenarios and may not be exposed by initial testing of new features.
